...
class SimillerProductDetailVC: BaseViewController , UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        manager.POST("http://192.168.0.6/eshopkart/webservices/get_products", parameters: params, success: { (operation : AFHTTPRequestOperation!, response : AnyObject!) -> Void in
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> SimillerProductViewCell {
        ...
        //getQuoteBtn.tag = 101
        let url = NSURL(string:("http://192.168.0.6/eshopkart/files/thumbs100x100/" + (itemInfoDic["Gallery"]?.objectAtIndex(0)["images"] as? String)!))
